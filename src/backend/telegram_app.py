...
def _make_gh() -> "GitHubStore | None":
    ...
    if not token or not repo:
        return None
    return readclient.make_store(token, repo, branch)  # v3.8.2: 읽기는 백엔드 /fetch 경유
...
if _FLASK_AVAILABLE:
    ...
    def _telegram_webhook():
        ...
        try:
            ...
            gh = readclient.make_store(gh_token, gh_repo, gh_branch)  # v3.8.2: 읽기는 백엔드 /fetch 경유
            channels_cfg = _load_channels_config()
