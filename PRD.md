...
> **실행 스케줄 (cron `0 * * * *`, 매시간 어느 한 모드가 실행됨 · 시각은 JST=UTC+9 기준):**
> - **딥 스캔**: JST 18·19·20·21·22·23·00·01시 (예고 집중 시간대, 매시간) + JST 02·10·14시 (주간). 하루 **11회**.
> - **라이트 싱크**: 그 외 모든 시각(주간 대부분). 하루 13회.
> - 방송인들은 대체로 방송 1~2일 전, 저녁~자정 JST에 예고를 올리므로 그 시간대만 딥으로 촘촘히 커버.
...
| 작업 | 단가 | 횟수/일 | 소계 |
|---|---|---|---|
| 라이트 싱크 `videos.list` (배치 2회) | 1 | 13 × 2 | 26 |
| 딥 스캔 `search.list` (5채널) | 100 | 11 × 5 | 5,500 |
| 딥 스캔 `videos.list` (배치 2회) | 1 | 11 × 2 | 22 |
| **합계** | | | **≈ 5,548 / day** |

여유 ~45%. 수동 트리거(`workflow_dispatch mode=deep`) 여유분 포함.
