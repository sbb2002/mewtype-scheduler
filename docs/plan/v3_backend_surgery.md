...
- **별도 "collab" 상태를 만드는 게 아님.** 현행 6상태는 "한 멤버 레인의 방송 하나" 전제로
  쓰여 있는데, 합동은 참여 멤버 전원 레인에 같은 카드가 팬아웃된다. 따라서 6상태 각각에
  "이게 합동일 때" 의 추가 규칙(여러 레인 동기화)을 **상태 × collab여부 매트릭스**로 채워야 한다.
  칸별 미정 사항:
  - `announced` : 참여 멤버(`collab_with`)를 무엇으로 확정? 한 레인에만 vs 전원 레인에 아이템 생성?
  - `upcoming` : 승격 조건(날짜+시간+제목+썸네일+url) 판정을 참여자 중 누구 채널 기준으로?
    한 명 채널에 실물 뜨면 나머지 레인도 같이 승격?
  - `watching` : 3분 폴링을 참여자 전원 채널에 도나, 호스트 채널만?
  - `live` : v2.6 규칙(참여자 중 아무 채널에나 실물 → 확정 + `_carry_collab` 로 나머지 레인 이관)이
    6상태에서 언제 발동하나.
  - `end` : 방송 종료 시 팬아웃된 여러 레인이 **동시에** `end→none`. 한 레인만 내려가고 다른
    레인에 유령이 남으면 안 됨. "다시 켜짐" 판정도 전원 공통.
  - (소멸) : `host="group"`(`parse_appearance` 出演情報, 외부 이벤트) 특례 = supersede 안 함.
