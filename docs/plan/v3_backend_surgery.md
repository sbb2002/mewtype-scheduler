...
## 합동방송(collab) — 6상태 모델에 재매핑 필요
- 감지 방식은 v2.4/2.6 그대로: 개인 예고 트윗의 url + 본문 내용, 공식 채널은 본문(url 은
  불확실하나 내용상 주어질 확률 높음). **확인항목**: 개인 예고 트윗이 collab 판별에 쓸 url/맥락을
  실제로 실어주는지 실물 샘플로 검증.
- 미작성: `announced→upcoming→watching→live→end` 각 단계에서 참여 멤버 레인 팬아웃
  (`collab_with`, `_carry_collab`, `host="group"` 특례)이 어떻게 유지되는지 + supersede 가
  참여자 집합에 대해 어떻게 도는지. v3 상태 전이표에 collab 축을 넣어야 한다.
...
## 마이그레이션 순서·롤백 (draft "데이터 브랜치 재구성" 관련)
- "마이그레이션 시점" = v3 출시 때 `data` 브랜치 JSON(`schedule.json`·`notices.json`·`tweets.json`)이
  v2 스키마 → v3 스키마로 바뀌는 커밋. 프론트가 이 파일을 raw CDN 에서 계속 읽으므로 구 프론트가
  신 스키마를(또는 그 반대) 만나는 창이 생긴다 (raw 캐시 ~5분 + Vercel 배포 타이밍).
- 후보안:
  - (a) 신 프론트를 두 스키마 다 견디게(모르는 status→`announced` 취급, 없는 필드 폴백) 만들어
    먼저 배포 → data 재구성 커밋 → 구 코드 정리. **권장**.
  - (b) `/pause` 후 data 재구성 + 프론트 배포 동시 → `/resume`. 몇 분 점검 감수.
  - (c) 파일명 버저닝(`schedule.v3.json`) + 프론트 배포에서 `DATA_URL` 교체.
- 롤백: 코드는 이전 리비전으로, data 는 `.old/` 백업에서 복원. 절차를 golive 런북에 명시.
