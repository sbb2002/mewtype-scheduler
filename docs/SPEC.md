...
def match_item(items, inc, *, superscede_sec=45*60) -> dict | None       # §1-1
...
트리거 📥 = `upstream` 건수. 그날 이벤트가 전부 `[백필]`(실시간 모니터링 2026-09-15 이전)이면 상세에 `backfillOnly: true`,
요약 항목은 `{"backfill_only": true, "events": N}` — 트리거 수를 단정하지 않는다(잔디 `?`).
