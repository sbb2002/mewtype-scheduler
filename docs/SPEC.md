...
- **용어집(`GLOSSARY`, v3.1.14)**: 고유명사가 호출마다 다르게 번역되는 문제(실측: 그룹명
  ...
  `{"夢限大みゅーたいぷ": "무겐다이 뮤타입", "バンドリ！/バンドリ!/バンドリ": "뱅드림!"}`
  (v3.1.15 — "バンドリ"가 "밴드리"로 오역되던 것 추가 등록. 느낌표 포함/미포함 변형을
  긴 패턴부터 순서대로 등록해 이중 처리 방지. 토큰이 영숫자에 바로 들러붙으면(예:
  "バンドリ13th") LLM 이 뒤 문장을 통째로 미번역으로 남기는 사례가 있어 그 경우만
  공백을 끼워 넣는다).
