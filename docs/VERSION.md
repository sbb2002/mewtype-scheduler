...
- **v3.1.6** (핫픽스) — 폰(Automate)이 이모지(서로게이트쌍 필요한 문자) 처리 중 트윗 원문 바이트를
  깨뜨려 보내던 것 발견 — 치환문자·짝 없는 서로게이트가 섞여 들어와 저장도 파싱도 깨졌음
  (서버에서 복구 불가). `/ingest`가 손상·잘림 흔적을 감지하면 같은 트윗을 vxtwitter로
  다시 조회해 원문 전체를 교체하도록 함 — 소식·스케줄·개인트윗 모든 경로가 항상 온전한
  원문을 받도록.
