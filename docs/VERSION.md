...
- **v3.1.15** — 용어집(`GLOSSARY`)에 "バンドリ"("밴드리"로 오역되던 것) → "뱅드림!" 추가.
  느낌표 포함/미포함 세 변형을 긴 패턴부터 순서대로 등록해 이중 느낌표("!!")를 방지하고,
  토큰이 영숫자에 바로 들러붙어(예: "バンドリ13th") LLM 이 뒤 문장을 통째로 미번역으로
  남기던 부작용도 그 경우에만 공백을 끼워 넣어 수정. 이미 커밋돼 있던 notices.json 4건의
  "밴드리" 표기도 "뱅드림!"으로 소급 정정.
