...
- **v3.8.0b** (기능, 대안·미배포) — v3.8.0a 와 같은 백엔드·저장 스키마(`video`)를 쓰되, 프론트만 **X 공식 임베드**로 재생.
  `<video src>` 직접 재생(a)과 달리 영상 파일 주소·Referer 우회(`no-referrer` 메타)를 쓰지 않는다. 브랜치 `feat/v3.8.0b-tweet-embed`.
  1. **동작** — 평소엔 썸네일 + "X에서 영상 재생"(길이 · X) 버튼만 보이고, 누를 때에만 X 공식 임베드 iframe
     (`platform.twitter.com/embed/Tweet.html?id=<트윗 id>&theme=dark&dnt=true&lang=ko&hideThread=true&frame=false`)을 만든다. X 스크립트·쿠키를
     방문자 전체에게 로드하지 않기 위함. 임베드는 **트윗 전체**(프로필·팔로우 버튼·본문·영상·참조 트윗)를 그린다 — 영상만 보여주는 옵션은 찾지 못했다.
  2. **실측으로 정한 제약** — ① X 임베드는 폭 **250px 이상**이어야 렌더링 후 `twttr.private.resize`(자동 높이) 메시지를 보낸다(폭 184·232px 에서는
     안 옴). PC 말풍선 스레드 영역(≈248px)이 좁아 임베드가 떠 있는 동안만 말풍선을 296px 로 넓히고(`has-embed`), 임베드는 메시지 말풍선 밖(스레드
     전체 폭)에 붙인다. ② 이 트윗은 폭 250px 에서 높이 1036px 을 요청 → 상한 520px 만 적용하고 나머지는 iframe 안쪽 스크롤. ③ 첫 로딩에 약 10초(영상 포함).
  3. **정리/예외** — 말풍선·시트를 닫으면 임베드를 지운다(다시 열면 썸네일 상태). 합성 id(숫자 아님)는 임베드할 수 없어 버튼이 원문 링크로 이동.
     재생 실패는 iframe(cross-origin)이라 감지할 수 없어 X 임베드 자체의 안내에 맡긴다.
  4. **a 대비** — 디자인: 말풍선 스타일을 벗어나 트윗 카드가 통째로 나옴 / 정책: X 가 공식 제공하는 방식(Referer 우회 없음) / 무게: X 스크립트·쿠키 로드 +
     첫 로딩 약 10초. `index.html` 의 `no-referrer` 메타는 불필요해 제거.
  5. **검증** — 로컬 프론트 + 실제 저장 데이터 픽스처로 Chrome 에서 확인: 임베드 렌더링·재생, 폭 260px 에서 자동 높이(520px 상한) 적용, 닫았다 열면 정리.
     미확인: 모바일 시트, Safari, 실제 GIF 트윗.
