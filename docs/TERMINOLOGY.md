...
| **DB 관제소** | (v3.8.2) `data` 저장소(GitHub) 접근이 지나가는 유일한 문. 제어 채널·백엔드의 읽기·쓰기를 한 서비스(`mewtype-db-tower`)의 FIFO 큐로 모아 직렬 처리하고, 속도 제한(403/429)이 걸리면 기다렸다 재시도하며, 대기 상한을 넘기면 503 으로 돌려준다. 개별 GitHub 호출의 순서·속도만 다루고 트랜잭션 직렬화(`/write` 잡)는 백엔드가 맡는다. | 큐 서버, 프록시, DB 서버 |
